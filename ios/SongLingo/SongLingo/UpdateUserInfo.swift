...
struct UpdateUserInfo: View {
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            Button(action: {
                if title == "Password" {
                    // Check if the old password matches only when changing password
                    ...
                    if isMinLength && hasLetter && hasUppercase && hasSpecialChar && hasNumber {
...
struct RequirementRow: View {
    ...
    var body: some View {
        HStack {
            Image(systemName: isMet ? "checkmark.circle" : "circle")
...
struct Preview: PreviewProvider {
    ...
    static var previews: some View {
        NavigationView {
            UpdateUserInfo(value: $sampleValue, title: "Email", currentPass: "Happy123!")
        }
    }
}
